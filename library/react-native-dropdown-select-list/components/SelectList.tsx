...
import { wScale } from '../../../src/styles/Scale'

import { SelectListProps } from '..';

type L1Keys = { key?: any; value?: any; img?: any; isLoading?: boolean; disabled?: boolean | undefined }
        ...
        <View>
            {
                (dropdown && search)
                    ...
                    <TouchableOpacity style={[styles.wrapper, boxStyles]} onPress={() => { if (!dropdown) { Keyboard.dismiss(); slidedown() } else { slideup() } }}>
                        ...
                        <View style={{ width: 1, height: 24, backgroundColor: Colors.gray, marginRight: wScale({ size: 10 }), marginLeft: wScale({ size: -7 }) }} />
                        {selectedimg && <Image source={{ uri: selectedimg, width: 30, height: 30 }} style={{ resizeMode: 'contain', marginRight: 10 }} />}
                        <Text style={[{ fontFamily, color: Colors.main }, inputStyles]}>{(selectedval == "") ? (placeholder) ? placeholder : '' : selectedval}</Text>
                        {
                            (!arrowicon)
                                ?
                                <Image
                                            ...
                                            return (
                                                <TouchableOpacity style={[styles.option, dropdownItemStyles]} key={index} onPress={() => {
                                                    ...
                                                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                                        <Image source={{ uri: img, width: 30, height: 30 }} style={{ resizeMode: 'contain', marginRight: 10 }} />
